...
export async function call(model, prompt, { maxTokens = 2048, temperature = null, system = null } = {}) {
  ...
  let ttft = null, usage = null, err = null, content = '', reasoned = false;
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    t0 = performance.now();
    ttft = null; usage = null; err = null; content = ''; reasoned = false;
  ...
  return {
    ...
    failure: classifyFailure(err, content, usage),
  };
}

function classifyFailure(err, content, usage) {
  if (err) {
  ...
  // truncation: finished because it ran out of tokens (heuristic; usage has no finish_reason here)
  return null; // ok
}
